...
function instrument(page) {
  ...
  page.on(
    "console",
    (m) => m.type() === "error" && page.__errs.push(m.text().slice(0, 110)),
  );
  page.on("pageerror", (e) =>
    page.__errs.push(`pageerror: ${String(e).slice(0, 110)}`),
  );
  page.on("requestfailed", (r) =>
    page.__errs.push(`requestfailed: ${r.url().slice(0, 70)}`),
  );
  return page;
}
...
for (const [name, url, auth] of [
  ...
  for (const [label, vp] of [
    ...
    try {
      ...
      console.log(
        `${(name + " · " + label).padEnd(40)} FAILED: ${String(e).split("\n")[0].slice(0, 60)}`,
      );
  ...
  for (const [label, vp] of [
    ...
    try {
      ...
      for (const section of [
        ...
        await page.goto(`${TELLER}/t/${tenant}/${section}`, {
      ...
      await page.goto(`${TELLER}/t/${tenant}/members`, {
        waitUntil: "networkidle",
      });
      f = page.frame({ name: "mainFrame" });
...
for (const [n, l, e] of problems)
  console.log(`  ${n} (${l}): ${e.join(" | ")}`);
