 ...
 * text claims exist.
 */
const DOCS = ["README.md", "REPORT.md", "DECISIONS.md", "evidence/README.md", "deploy/README.md"];
const problems = [];
const ok = (m) => console.log(`  OK    ${m}`);
const bad = (m) => { console.log(`  BAD   ${m}`); problems.push(m); };
...
for (const doc of DOCS) {
  if (!existsSync(doc)) { bad(`${doc} is missing`); continue; }
  const text = readFileSync(doc, "utf8");
  const base = doc.includes("/") ? doc.split("/").slice(0, -1).join("/") + "/" : "";

  for (const m of text.matchAll(/\]\((?!https?:|#|mailto:)([^)\s#]+)/g)) {
    const p = m[1].replace(/\/$/, "");
    if (!existsSync(p) && !existsSync(base + p)) bad(`${doc}: link to missing path "${m[1]}"`);
  }

  for (const m of text.matchAll(/`((?:src|web|target-app|scripts|tests|deploy|artifacts|evidence|docs)\/[^`\s]+)`/g)) {
    const p = m[1].replace(/\/$/, "");
  ...
  if (doc !== "DECISIONS.md") {
    for (const m of text.matchAll(/npm run ([a-z:]+)/g)) {
      if (!scripts.includes(m[1])) bad(`${doc}: "npm run ${m[1]}" is not a script`);
    }
  }
}
ok(`${DOCS.length} docs scanned for paths and commands`);

const testOut = stripAnsi(execSync("npx vitest run 2>&1 || true", { encoding: "utf8" }));
...
else if (claimed !== realTests) bad(`README claims ${claimed} unit tests, actual is ${realTests}`);
else ok(`test count matches (${realTests})`);

for (const m of readme.matchAll(/cu\.member\.[a-z_]+(@\d+\.\d+\.\d+)?/g)) {
  const [id, ver] = m[0].split("@");
  const hits = execSync(`ls artifacts/ | grep -c "^${id}@" || true`, { encoding: "utf8" }).trim();
  if (hits === "0") bad(`README names capability "${m[0]}" with no artifact`);
  else if (ver && !existsSync(`artifacts/${id}@${ver}.json`)) bad(`README names "${m[0]}" but that version is absent`);
}
...
for (const m of evReadme.matchAll(/`(\d\d-[a-z-]+)`/g)) {
  if (!existsSync(`evidence/${m[1]}`)) bad(`evidence/README.md describes missing run "${m[1]}"`);
}
ok("evidence runs described all present");

for (const m of readme.matchAll(/\(docs\/screenshots\/([^)]+)\)/g)) {
  if (!existsSync(`docs/screenshots/${m[1]}`)) bad(`README embeds missing screenshot "${m[1]}"`);
}
